...
<button type="button"
        ...
        class="{{ $buttonClass }} cursor-pointer  transition:transform hover:scale-105">
    {{ $buttonText }}
</button>
...
<dialog id="{{ $modalId }}" class="category-modal rounded-3xl border border-slate-800 bg-slate-900 p-0 text-slate-200 shadow-2xl">
    <div class="w-full max-w-2xl p-6">
        <div class="flex items-start justify-between gap-4">
            <div>
                ...
                <p class="mt-2 text-sm leading-6 text-slate-400">
                    Add a new category by entering the name and description below.
                </p>
            </div>
        ...
        <form method="post" action="{{ $action }}" class="mt-6 space-y-5">
            ...
            <div class="space-y-2">
                <label for="create-name-{{ $modalId }}" class="text-sm font-medium text-slate-300">Category name</label>
                <input id="create-name-{{ $modalId }}"
                       type="text"
                       name="name"
                       value="{{ $name }}"
            ...
            <div class="space-y-2">
                <label for="create-description-{{ $modalId }}" class="text-sm font-medium text-slate-300">Description</label>
                <textarea id="create-description-{{ $modalId }}"
                          name="description"
                          rows="5"
                          class="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-white outline-none transition focus:border-slate-500"
